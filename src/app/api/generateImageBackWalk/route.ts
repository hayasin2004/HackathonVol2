...
- Upper Clothing: ${upperClothing} in ${upperClothingColor} color.`;
    ...
    const motionInstruction = "Back walk motion: Generate an image showing the character walking from a back view. One arm and one leg should swing forward while all other details remain unchanged.";
    ...
    if (!generatedBuffer) throw new Error("No image data received for back walk motion.");

    const newFileName = originalFileName.replace("back", "back_walk");
    ...
    console.error("Error in generateImageBackWalk API route:", error);
