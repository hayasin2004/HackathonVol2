...
- Upper Clothing: ${upperClothing} in ${upperClothingColor} color.`;
    ...
    const directionInstruction = "後ろ画像生成して！ The character should be depicted from a back view, keeping all details exactly the same.";
    ...
    if (!generatedBuffer) throw new Error("No image data received for back view.");

    const newFileName = originalFileName.replace("front", "back");
    ...
    console.error("Error in generateImageBack API route:", error);
