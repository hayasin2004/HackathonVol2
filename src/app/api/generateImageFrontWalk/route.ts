...
- Upper Clothing: ${upperClothing} in ${upperClothingColor} color.`;
    ...
    const motionInstruction = "Front walk motion: Generate an image showing the character walking. One arm and one leg should swing forward while all other details remain unchanged.";
    ...
    if (!generatedBuffer) throw new Error("No image data received for front walk motion.");

    const newFileName = originalFileName.replace("front", "front_walk");
    ...
    console.error("Error in generateImageFrontWalk API route:", error);
