...
- Upper Clothing: ${upperClothing} in ${upperClothingColor} color.`;
    ...
    const directionInstruction = "Character facing right. Profile view from the right side, showing the character looking towards the right side of the frame. Maintain all original details of the character's design, clothing, and style, only changing the viewing angle.";
    ...
    if (!generatedBuffer) throw new Error("No image data received for right view.");

    const newFileName = originalFileName.replace("front", "right");
    ...
    console.error("Error in generateImageRight API route:", error);
