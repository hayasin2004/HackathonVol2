...
- Upper Clothing: ${upperClothing} in ${upperClothingColor} color.`;
    ...
    const motionInstruction = "Right walk motion: Generate an image showing the character walking from a right-side view. One arm and one leg should swing forward while all other features remain unchanged.";
    ...
    if (!generatedBuffer) throw new Error("No image data received for right walk motion.");

    const newFileName = originalFileName.replace("right", "right_walk");
    ...
    console.error("Error in generateImageRightWalk API route:", error);
