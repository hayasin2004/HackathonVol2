...
const RoomPage = ({ params }: { params: { id: string } }) => {
  ...
  const handleTKeyPress = useCallback(() => {
    ...
    if (currentMap === "grass") {
      //grassmapの座標に居るかの確認
      const isPlayerInGrassDesertMapCoordinates =
        grassdesertMapCoordinates.some(
          (coord) => coord.x === playerX && coord.y === playerY
        );
      const isPlayerInGrassSnowMapCoordinates = grasssnowMapCoordinates.some(
        (coord) => coord.x === playerX && coord.y === playerY
      );
      ...
      if (event.key === "t" || event.key === "T") {
        ...
        if (userId) {
          const currentUserId = async () => {
            const ItemResponse = await fetch(`/api/item/fetchItem`, {
              method: "GET",
            });
            const response = await fetch(`/api/player/catch/${userId}`, {
              method: "GET",
            });
            if (!response.ok) {
    ...
    const initializePlayerData = async () => {
      try {
        ...
        const checkResponse = await fetch(
          `/api/player/check?playerId=${playerId.id}`,
          { method: "GET" }
        );
